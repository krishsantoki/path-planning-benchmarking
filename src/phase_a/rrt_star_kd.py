...
class RRTStarKD:
    """RRT* algorithm with KD-Tree optimized nearest neighbor search."""
    ...
    def search(self, start, goal):
        ...
        tree = [start]
        tree_array = [list(start)]  # For KDTree rebuilds
        parent = {start: None}
        cost = {start: 0.0}
        ...
        # Rebuild KDTree every N iterations for efficiency
        kd_rebuild_interval = 50
        kd_tree = KDTree([list(start)])
        ...
        for i in range(self.max_iterations):
            ...
            # KD-Tree nearest neighbor — O(log n) instead of O(n)
            _, nearest_idx = kd_tree.query(list(random_point))
            ...
            # KD-Tree range search for nearby nodes — O(log n) instead of O(n)
            nearby_indices = kd_tree.query_ball_point(list(new_node), self.rewire_radius)
            nearby = [tree[idx] for idx in nearby_indices]
            ...
            if len(tree) % kd_rebuild_interval == 0:
