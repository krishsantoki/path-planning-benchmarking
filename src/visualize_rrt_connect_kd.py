...
def visualize_static(grid, result, title="RRT-Connect (KD-Tree)"):
    ...
    plt.tight_layout()
    plt.savefig("visualizations/rrt_connect_kd_result.png", dpi=150, bbox_inches='tight')
    plt.show()
    print("Saved to visualizations/rrt_connect_kd_result.png")


def visualize_animated(grid, result, title="RRT-Connect (KD-Tree)", filename="rrt_connect_kd_animation.gif"):
    ...
    edges_start = [(par, node) for node, par in result["parent_start"].items() if par is not None]
    edges_goal = [(par, node) for node, par in result["parent_goal"].items() if par is not None]
